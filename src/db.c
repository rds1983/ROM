...
void load_area( FILE *fp )
{
    ...
    pArea->lvnum        = fread_number( fp );                    /* OLC */
    pArea->uvnum        = fread_number( fp );                    /* OLC */
...
void new_load_area( FILE *fp )
{
    ...
    pArea->credits      = str_dup( "{00 00} OLC New Area" );
...
void load_rooms( FILE *fp )
{
 ...
 pexit->rs_flags = EX_ISDOOR | EX_CLOSED | EX_LOCKED; break;
		case 3:
 pexit->rs_flags = EX_ISDOOR | EX_CLOSED | EX_LOCKED | EX_NOPASS; break;
		case 4:
 pexit->rs_flags = EX_ISDOOR | EX_CLOSED | EX_LOCKED | EX_PICKPROOF; break;
		case 5:
 pexit->rs_flags = EX_ISDOOR | EX_CLOSED | EX_LOCKED | EX_PICKPROOF | EX_NOPASS; break;
		}
...
void load_resets( FILE *fp )
{
    ...
    for ( ; ; )
    {
		...
		case 3: SET_BIT( pexit->rs_flags, EX_ISDOOR|EX_CLOSED|EX_LOCKED|EX_NOPASS ); break;
...
void reset_room( ROOM_INDEX_DATA *pRoom )
{
	...
	int count;
	    ...
	    if ( !(pRoomIndex = get_room_index( pReset->arg3 ) ) )
	    {
                bug( "Reset_room: 'M': bad room vnum %d.", pReset->arg3 );
                continue;
            }
